...
_install_php_depend(){
    ...
    mkdir -p ${php72_location} > /dev/null 2>&1
}
...
rpminstall_php72(){
    _GetRPMArch
    rpm_package_name="php72-7.2.27-1.${RPMArch}.x86_64.rpm"
    php72_location=/hws.com/hwsmaster/server/php-7_2_27
    _install_php_depend
    DownloadUrl ${rpm_package_name} ${download_root_url}/rpms/${rpm_package_name}
    CheckError "rpm -ivh ${rpm_package_name} --force --nodeps"
    kill -9 -`cat ${php72_location}/var/run/default.pid`
    _config_php
    /etc/init.d/php72 start
    /etc/init.d/php72 restart
}

debinstall_php72(){
    deb_package_name="php-7.2.27-linux-amd64.deb"
    _install_php_depend
