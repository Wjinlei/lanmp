...
_install_php_depend(){
    ...
    mkdir -p ${php55_location} > /dev/null 2>&1
}
...
rpminstall_php55(){
    ...
    kill -9 -`cat ${php55_location}/var/run/default.pid`
    _config_php
    /etc/init.d/php55 start
    /etc/init.d/php55 restart
}

debinstall_php55(){
    deb_package_name="php-5.5.38-linux-amd64.deb"
    _install_php_depend
