...
_install_php_depend(){
    ...
    mkdir -p ${php80_location} > /dev/null 2>&1
}
...
rpminstall_php80(){
    ...
    kill -9 -`cat ${php80_location}/var/run/default.pid`
    _config_php
    /etc/init.d/php80 start
    /etc/init.d/php80 restart
}

debinstall_php80(){
    deb_package_name="php-8.0.0-linux-amd64.deb"
    _install_php_depend
