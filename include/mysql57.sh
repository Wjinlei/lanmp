...
_config_mysql(){
    chown -R mysql:mysql ${mysql57_location} ${mysql_data_location}
    sed -i "s:^basedir=.*:basedir=${mysql57_location}:g" ${mysql57_location}/support-files/mysql.server
    sed -i "s:^datadir=.*:datadir=${mysql_data_location}:g" ${mysql57_location}/support-files/mysql.server
...
EOF
    ...
    rm -f /tmp/${mysql57_filename}.tar.gz
