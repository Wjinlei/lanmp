...
_config_php(){
    ...
    # disable_functions
    php_disable_functions=`grep "^disable_functions" ${php70_location}/etc/php.ini |wc -l`
    if [[ ${php_disable_functions} -eq 0 ]];then
        echo "disable_functions = system, passthru, exec, chroot, chgrp, chown, proc_get, shell_exec, popen, escapeshellarg, escapeshellcmd, proc_close, proc_open, dl, proc_get_status, ini_alter, ini_alter, ini_restore" >> ${php70_location}/etc/php.ini
    else
        sed -i '/^disable_functions/ s/.*/disable_functions = system, passthru, exec, chroot, chgrp, chown, proc_get, shell_exec, popen, escapeshellarg, escapeshellcmd, proc_close, proc_open, dl, proc_get_status, ini_alter, ini_alter, ini_restore/' ${php70_location}/etc/php.ini
    fi

    # mysqld
    if [[ -d "${mysql_data_location}" ]]; then
        sock_location=/tmp/mysql.sock
        sed -i "s#mysql.default_socket.*#mysql.default_socket = ${sock_location}#" ${php70_location}/etc/php.ini
        sed -i "s#mysqli.default_socket.*#mysqli.default_socket = ${sock_location}#" ${php70_location}/etc/php.ini
        sed -i "s#pdo_mysql.default_socket.*#pdo_mysql.default_socket = ${sock_location}#" ${php70_location}/etc/php.ini
    fi

    # default_charset
    php_default_charset=`grep "^default_charset" ${php70_location}/etc/php.ini |wc -l`
    if [[ ${php_default_charset} -eq 0 ]];then
        echo 'default_charset = "UTF-8"' >> ${php70_location}/etc/php.ini
    else
        sed -i 's/^default_charset.*/default_charset = "UTF-8"/g' ${php70_location}/etc/php.ini
    fi

    # short_open_tag
    short_open_tag=`grep "^short_open_tag" ${php70_location}/etc/php.ini |wc -l`
    if [[ ${short_open_tag} -eq 0 ]];then
        echo 'short_open_tag = On' >> ${php70_location}/etc/php.ini
    else
        sed -i 's/^short_open_tag.*/short_open_tag = On/g' ${php70_location}/etc/php.ini
    fi

    # always_populate_raw_post_data = -1
    always_populate_raw_post_data=`grep "^always_populate_raw_post_data" ${php70_location}/etc/php.ini |wc -l`
    if [[ ${always_populate_raw_post_data} -eq 0 ]];then
        echo 'always_populate_raw_post_data = -1' >> ${php70_location}/etc/php.ini
    else
...
install_php70(){
    mkdir -p ${backup_dir}
    if [ -d "${php70_location}" ]; then 
        for pidfile in `ls ${php70_location}/var/run/`
        do
            kill -INT `cat ${php70_location}/var/run/${pidfile}`
        done
        if [ -d "${backup_dir}/${php70_install_path_name}" ]; then
            mv ${backup_dir}/${php70_install_path_name} ${backup_dir}/${php70_install_path_name}-$(date +%Y-%m-%d_%H:%M:%S).bak
        fi
        mv ${php70_location} ${backup_dir}
    fi
    _install_php_depend
    cd /tmp
    ...
    CheckError "make install"
    #if [ -d "${backup_dir}/${php70_install_path_name}" ]; then
        #if [ -d "${backup_dir}/${php70_install_path_name}/etc" ]; then
            #rm -fr ${php70_location}/etc
            #cp -fr ${backup_dir}/${php70_install_path_name}/etc ${php70_location}
        #fi
        #if [ -d "${backup_dir}/${php70_install_path_name}/php.d" ]; then
            #rm -fr ${php70_location}/php.d
            #cp -fr ${backup_dir}/${php70_install_path_name}/php.d ${php70_location}
        #fi
        #if [ -d "${backup_dir}/${php70_install_path_name}/lib" ]; then
            #rm -fr ${php70_location}/lib
            #cp -fr ${backup_dir}/${php70_install_path_name}/lib ${php70_location}
        #fi
    #else
