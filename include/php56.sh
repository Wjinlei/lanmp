...
_config_php(){
    ...
    # disable_functions
    php_disable_functions=`grep "^disable_functions" ${php56_location}/etc/php.ini |wc -l`
    if [[ ${php_disable_functions} -eq 0 ]];then
        echo "disable_functions = system, passthru, exec, chroot, chgrp, chown, proc_get, shell_exec, popen, escapeshellarg, escapeshellcmd, proc_close, proc_open, dl, proc_get_status, ini_alter, ini_alter, ini_restore" >> ${php56_location}/etc/php.ini
    else
        sed -i '/^disable_functions/ s/.*/disable_functions = system, passthru, exec, chroot, chgrp, chown, proc_get, shell_exec, popen, escapeshellarg, escapeshellcmd, proc_close, proc_open, dl, proc_get_status, ini_alter, ini_alter, ini_restore/' ${php56_location}/etc/php.ini
    fi

    # mysqld
    if [[ -d "${mysql_data_location}" ]]; then
        sock_location=/tmp/mysql.sock
        sed -i "s#mysql.default_socket.*#mysql.default_socket = ${sock_location}#" ${php56_location}/etc/php.ini
        sed -i "s#mysqli.default_socket.*#mysqli.default_socket = ${sock_location}#" ${php56_location}/etc/php.ini
        sed -i "s#pdo_mysql.default_socket.*#pdo_mysql.default_socket = ${sock_location}#" ${php56_location}/etc/php.ini
    fi

    # default_charset
    php_default_charset=`grep "^default_charset" ${php56_location}/etc/php.ini |wc -l`
    if [[ ${php_default_charset} -eq 0 ]];then
        echo 'default_charset = "UTF-8"' >> ${php56_location}/etc/php.ini
    else
        sed -i 's/^default_charset.*/default_charset = "UTF-8"/g' ${php56_location}/etc/php.ini
    fi

    # short_open_tag
    short_open_tag=`grep "^short_open_tag" ${php56_location}/etc/php.ini |wc -l`
    if [[ ${short_open_tag} -eq 0 ]];then
        echo 'short_open_tag = On' >> ${php56_location}/etc/php.ini
    else
        sed -i 's/^short_open_tag.*/short_open_tag = On/g' ${php56_location}/etc/php.ini
    fi

    # always_populate_raw_post_data = -1
    always_populate_raw_post_data=`grep "^always_populate_raw_post_data" ${php56_location}/etc/php.ini |wc -l`
    if [[ ${always_populate_raw_post_data} -eq 0 ]];then
        echo 'always_populate_raw_post_data = -1' >> ${php56_location}/etc/php.ini
    else
...
EOF
    ...
    ${php56_location}/sbin/php-fpm -y ${php56_location}/etc/default.conf >/dev/null 2>&1
...
install_php56(){
    mkdir -p ${backup_dir}
    if [ -d "${php56_location}" ]; then 
        for pidfile in `ls ${php56_location}/var/run/`
        do
            kill -INT `cat ${php56_location}/var/run/${pidfile}`
        done
        if [ -d "${backup_dir}/${php56_install_path_name}" ]; then
            mv ${backup_dir}/${php56_install_path_name} ${backup_dir}/${php56_install_path_name}-$(date +%Y-%m-%d_%H:%M:%S).bak
        fi
        mv ${php56_location} ${backup_dir}
    fi
    _install_php_depend
    cd /tmp
    ...
    CheckError "make install"
    #if [ -d "${backup_dir}/${php56_install_path_name}" ]; then
        #if [ -d "${backup_dir}/${php56_install_path_name}/etc" ]; then
            #rm -fr ${php56_location}/etc
            #cp -fr ${backup_dir}/${php56_install_path_name}/etc ${php56_location}
        #fi
        #if [ -d "${backup_dir}/${php56_install_path_name}/php.d" ]; then
            #rm -fr ${php56_location}/php.d
            #cp -fr ${backup_dir}/${php56_install_path_name}/php.d ${php56_location}
        #fi
        #if [ -d "${backup_dir}/${php56_install_path_name}/lib" ]; then
            #rm -fr ${php56_location}/lib
            #cp -fr ${backup_dir}/${php56_install_path_name}/lib ${php56_location}
        #fi
    #else
