...
_config_php(){
    ...
    sed -i 's/;date.timezone =.*/date.timezone = PRC/g' ${php52_location}/etc/php.ini
    sed -i 's/short_open_tag =.*/short_open_tag = On/g' ${php52_location}/etc/php.ini
    sed -i 's/expose_php =.*/expose_php = Off/g' ${php52_location}/etc/php.ini
    sed -i 's/;cgi.fix_pathinfo=.*/cgi.fix_pathinfo=1/g' ${php52_location}/etc/php.ini
    sed -i 's/max_execution_time =.*/max_execution_time = 300/g' ${php52_location}/etc/php.ini
    if [ -f /etc/pki/tls/certs/ca-bundle.crt ]; then
        sed -i 's#;curl.cainfo =.*#curl.cainfo = /etc/pki/tls/certs/ca-bundle.crt#g' ${php52_location}/etc/php.ini
        sed -i 's#;openssl.cafile=.*#openssl.cafile=/etc/pki/tls/certs/ca-bundle.crt#g' ${php52_location}/etc/php.ini
    elif [ -f /etc/ssl/certs/ca-certificates.crt ]; then
        sed -i 's#;curl.cainfo =.*#curl.cainfo = /etc/ssl/certs/ca-certificates.crt#g' ${php52_location}/etc/php.ini
        sed -i 's#;openssl.cafile=.*#openssl.cafile=/etc/ssl/certs/ca-certificates.crt#g' ${php52_location}/etc/php.ini
    fi
    sed -i 's/disable_functions =.*/disable_functions = passthru,exec,system,chroot,chgrp,chown,shell_exec,popen,proc_open,pcntl_exec,ini_alter,ini_restore,dl,openlog,syslog,popepassthru,pcntl_alarm,pcntl_fork,pcntl_waitpid,pcntl_wait,pcntl_wifexited,pcntl_wifstopped,pcntl_wifsignaled,pcntl_wifcontinued,pcntl_wexitstatus,pcntl_wtermsig,pcntl_wstopsig,pcntl_signal,pcntl_signal_dispatch,pcntl_get_last_error,pcntl_strerror,pcntl_sigprocmask,pcntl_sigwaitinfo,pcntl_sigtimedwait,pcntl_exec,pcntl_getpriority,pcntl_setpriority,imap_open,apache_setenv/g' ${php52_location}/etc/php.ini
...
[ZendOptimizer]
zend_extension=${extension_dir}/ZendOptimizer.so
zend_optimizer.optimization_level=15
EOF

    # php-fpm
    cat > ${php52_location}/etc/php-fpm.conf<<EOF
<?xml version="1.0" ?>
<configuration>
    <section name="global_options">
        <value name="pid_file">$php52_location/logs/php-fpm.pid</value>
        <value name="error_log">$php52_location/logs/php-fpm.log</value>
    ...
    <workers>
        <section name="pool">
            ...
            <value name="listen_address">/tmp/$php52_filename-default.sock</value>
...
install_php52(){
    ...
    CheckError "rm -fr ${php52_location}"
    cd /tmp
    _info "Downloading and Extracting ${php52_filename} files..."
    DownloadFile  "${php52_filename}.tar.gz" "${php52_download_url}"
    rm -fr ${php52_filename}
    tar zxf ${php52_filename}.tar.gz
    # 打fpm补丁
    DownloadFile "${php52_fpm_filename}" "${php52_fpm_download_url}"
    gzip -cd $php52_fpm_filename | patch -d $php52_filename -p1
    # fix bug make: *** [ext/dom/node.lo] Error 1
    DownloadFile "${php52_fixbug_nodec_filename}.tar.gz" "${php52_fixbug_nodec_download_url}"
    tar zxf ${php52_fixbug_nodec_filename}.tar.gz
    patch -d ${php52_filename} -p0 < ${php52_fixbug_nodec_filename}.patch
    # fix bug make: *** [ext/gmp/gmp.lo] Error 1
    DownloadFile "${php52_fixbug_gmpc_filename}.tar.gz" "${php52_fixbug_gmpc_download_url}"
    ...
    php_configure_args="--prefix=${php52_location} \
    --with-config-file-path=${php52_location}/etc \
    --with-config-file-scan-dir=${php52_location}/php.d \
