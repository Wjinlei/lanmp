...
_config_php(){
    ...
    cp -f php.ini-production ${php73_location}/etc/php.ini

    # disable_functions
    php_disable_functions=`grep "^disable_functions" ${php73_location}/etc/php.ini |wc -l`
    if [[ ${php_disable_functions} -eq 0 ]];then
        echo "disable_functions = system, passthru, exec, chroot, chgrp, chown, proc_get, shell_exec, popen, escapeshellarg, escapeshellcmd, proc_close, proc_open, dl, proc_get_status, ini_alter, ini_alter, ini_restore" >> ${php73_location}/etc/php.ini
    else
        sed -i '/^disable_functions/ s/.*/disable_functions = system, passthru, exec, chroot, chgrp, chown, proc_get, shell_exec, popen, escapeshellarg, escapeshellcmd, proc_close, proc_open, dl, proc_get_status, ini_alter, ini_alter, ini_restore/' ${php73_location}/etc/php.ini
    fi

    # mysqld
    if [[ -d "${mysql_data_location}" ]]; then
        sock_location=/tmp/mysql.sock
        sed -i "s#mysql.default_socket.*#mysql.default_socket = ${sock_location}#" ${php73_location}/etc/php.ini
        sed -i "s#mysqli.default_socket.*#mysqli.default_socket = ${sock_location}#" ${php73_location}/etc/php.ini
        sed -i "s#pdo_mysql.default_socket.*#pdo_mysql.default_socket = ${sock_location}#" ${php73_location}/etc/php.ini
    fi

    # default_charset
    php_default_charset=`grep "^default_charset" ${php73_location}/etc/php.ini |wc -l`
    if [[ ${php_default_charset} -eq 0 ]];then
        echo 'default_charset = "UTF-8"' >> ${php73_location}/etc/php.ini
    else
        sed -i 's/^default_charset.*/default_charset = "UTF-8"/g' ${php73_location}/etc/php.ini
    fi

    # short_open_tag
    short_open_tag=`grep "^short_open_tag" ${php73_location}/etc/php.ini |wc -l`
    if [[ ${short_open_tag} -eq 0 ]];then
        echo 'short_open_tag = On' >> ${php73_location}/etc/php.ini
    else
        sed -i 's/^short_open_tag.*/short_open_tag = On/g' ${php73_location}/etc/php.ini
    fi

    # always_populate_raw_post_data = -1
    always_populate_raw_post_data=`grep "^always_populate_raw_post_data" ${php73_location}/etc/php.ini |wc -l`
    if [[ ${always_populate_raw_post_data} -eq 0 ]];then
        echo 'always_populate_raw_post_data = -1' >> ${php73_location}/etc/php.ini
    else
        sed -i 's/^always_populate_raw_post_data.*/always_populate_raw_post_data = -1/g' ${php73_location}/etc/php.ini
    fi

    extension_dir=$(${php73_location}/bin/php-config --extension-dir)
    cat > ${php73_location}/php.d/opcache.ini<<EOF
[opcache]
...
EOF
    ...
    # php-fpm
    cat > ${php73_location}/etc/default.conf<<EOF
[global]
...
install_php73(){
    mkdir -p ${backup_dir}
    if [ -d "${php73_location}" ]; then 
        for pidfile in `ls ${php73_location}/var/run/`
        do
            kill -INT `cat ${php73_location}/var/run/${pidfile}`
        done
        if [ -d "${backup_dir}/${php73_install_path_name}" ]; then
            mv ${backup_dir}/${php73_install_path_name} ${backup_dir}/${php73_install_path_name}-$(date +%Y-%m-%d_%H:%M:%S).bak
        fi
        mv ${php73_location} ${backup_dir}
    fi
    _install_php_depend
    cd /tmp
    _info "Downloading and Extracting ${php73_filename} files..."
    DownloadFile  "${php73_filename}.tar.gz" "${php73_download_url}"
    rm -fr ${php73_filename}
    tar zxf ${php73_filename}.tar.gz
    cd ${php73_filename}
    _info "Install ${php73_filename} ..."
    Is64bit && with_libdir="--with-libdir=lib64" || with_libdir=""
    php_configure_args="--prefix=${php73_location} \
    --with-config-file-path=${php73_location}/etc \
