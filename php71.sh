...
_install_php_depend(){
    ...
    mkdir -p ${php71_location} > /dev/null 2>&1
}
...
install_php71(){
    ...
    CheckError "rm -fr ${php71_location}"
    cd /tmp
...
rpminstall_php71(){
    _GetRPMArch
    rpm_package_name="php71-7.1.33-1.${RPMArch}.x86_64.rpm"
    php71_location=/hws.com/hwsmaster/server/php-7_1_33
    _install_php_depend
    ...
    kill -9 -`cat ${php71_location}/var/run/default.pid`
    _config_php
    /etc/init.d/php71 start
    /etc/init.d/php71 restart
}

debinstall_php71(){
    deb_package_name="php-7.1.33-linux-amd64.deb"
    _install_php_depend
